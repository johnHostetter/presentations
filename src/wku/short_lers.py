...
class LERSDiagram(Slide, MovingCameraScene):
    ...
    def draw(self, origin=ORIGIN, scale=1.0, target_scene=None, animate=True) -> VGroup:
        if target_scene is None:
            target_scene = self
        # note to self I copied this code directly from NoCodeGraph and only kept the relevant parts
    ...
    return SlideWithList(
        ...
        beamer_list=BL(
            items=[
                ...
                ItemizedList(
                    items=[
                        "First use of rough theory with fuzzy RL",
