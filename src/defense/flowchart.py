...
class Flowchart(Slide, MovingCameraScene):
    ...
    def construct(self):
        ...
        time_steps = sorted(
            [
                int(folder.stem)
                for folder in (self.image_dir / "optuna").iterdir()
                if folder.is_dir()
            ]
        )

        START_IDX: int = (
            397  # was 470 or 500 for 'test_images' directory (long is 4434)
        )
        END_IDX: int = 530  # was 530 for 'test_images' directory (long is 4499)
        image_paths = natsorted(
            (self.image_dir / "inputs" / "original_no_axes").iterdir()
        )[START_IDX:]
        resized_paths = natsorted(
            (self.image_dir / "inputs" / "resized_no_axes").iterdir()
        )[START_IDX:]
        ...
        for original_path, resized_path in zip(image_paths, resized_paths):
            ...
            left_text = (
                Text("Original Image", font_size=24, color=BLACK)
                .next_to(left_jpg, DOWN)
                .set_z_index(100)
            )  # bring to front with a very high z-index
            right_jpg = ImageMobject(resized_path).next_to(left_jpg, RIGHT)
            right_text = (
                Text("Resized Image", font_size=24, color=BLACK)
                .next_to(right_jpg, DOWN)
                .set_z_index(100)
            )  # bring to front with a very high z-index
            group_jpg = Group(
                Group(left_jpg, left_text), Group(right_jpg, right_text)
            ).move_to(ORIGIN)
            ...
            if curr_time_step in time_steps:  # stem is the time-step
                ...
                occlusion_jpg = (
                    ImageMobject(optuna_path / "occlusion.png")
                    .scale(0.89)
                    .move_to(right_jpg)
                    .shift(0.125 * DOWN)
                    .shift(4.53 * RIGHT)
                ...
                for conv_idx, conv_path in enumerate(conv_paths):
                    ...
                    if "matching" in conv_path.stem:
                        ...
                        conv_jpg = (
                            ImageMobject(conv_path).scale(0.5).move_to(right_most_jpg)
                        )
                        self.play(
                            ReplacementTransform(right_most_jpg, conv_jpg),
                        )
                    else:
                        conv_jpg = (
                            ImageMobject(conv_path)
                            .scale(0.5)
                            .next_to(right_most_jpg, RIGHT)
                            .shift(1.5 * RIGHT)
                        )
                        conv_text = Text(
                            f"After Convolutional Layer {conv_idx + 1}",
                            font_size=24,
                            color=BLACK,
                        ).next_to(conv_jpg, DOWN)
                        # TODO: fix this so it replaces the previous image
                        if (
                            f"{conv_path.parent.name}_{conv_idx}"
                            in self.objects_to_be_shifted
                        ):
                            self.play(
                ...
                INCREASE_HEIGHT_FACTOR: float = (
                    1.25  # increase the height of the camera frame
                )
                mu_centers_pattern = re.compile(
                    r"^center_\d+_exemplar$"
                )  # no need for .jpg since stem cuts it
                mu_degrees_pattern = re.compile(
                    r"^center_\d+_mu$"
                )  # no need for .jpg since stem cuts it
                rule_exemplar_pattern = re.compile(
                    r"^rule_\d+_exemplar$"
                )  # no need for .jpg since stem cuts it
                rule_mu_pattern = re.compile(
                    r"^rule_\d+_mu$"
                )  # no need for .jpg since stem cuts it
                right_most_jpg = self.add_mu_items(
                    mu_paths,
                    right_most_jpg,
                    regex_pattern=mu_centers_pattern,
                    text="Exemplary Percepts",
                )
                # objects_to_be_shifted.append(right_most_jpg)
                self.play(
                    self.camera.frame.animate.set(
                        height=right_most_jpg.get_height() * INCREASE_HEIGHT_FACTOR
                    )
                )
                self.wait(2)
                self.next_slide()
                right_most_jpg = self.add_mu_items(
                    mu_paths,
                    right_most_jpg[0][0],
                    regex_pattern=mu_degrees_pattern,
                ...
                right_most_jpg = self.add_mu_items(
                    mu_paths,
                    right_most_jpg[0][0],
                    regex_pattern=rule_exemplar_pattern,
                    text="Fuzzy Logic Rules",
                    idx_to_start_from=1,
                )
                # objects_to_be_shifted.append(right_most_jpg)
                self.play(
                ...
                right_most_jpg = self.add_mu_items(
                    mu_paths,
                    right_most_jpg[0][1],
                    regex_pattern=rule_mu_pattern,
                    text="Rule Applicability",
                    idx_to_start_from=1,
                )
                # objects_to_be_shifted.append(right_most_jpg)
                self.wait(2)
    ...
    def shift_objects(self, direction):
        if len(self.objects_to_be_shifted) > 0:
            self.play(
                Group(*self.objects_to_be_shifted.values()).animate.shift(
                    30 * direction
                ),
                Restore(self.camera.frame),
            )
    ...
    def add_mu_items(
        ...
        centers_text = Text(f"{text}", font_size=24, color=BLACK).next_to(
            centers_group, DOWN
        )
