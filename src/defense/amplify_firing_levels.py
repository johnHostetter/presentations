    ...
    def construct(self):
        ...
        for img_idx in [0, 2, 4]:
            for img_type in ["softmax", "entmax15", "norm_softmax", "norm_entmax15"]:
                sorted_img_path = (
                    self.image_dir / "sorted" / f"{img_type}_0_{img_idx}.png"
                )
                unsorted_img_path = (
                    self.image_dir / "unsorted" / f"{img_type}_0_{img_idx}.png"
                )
                ...
                if prev_images is None:
                    unsorted_img_caption = (
                        Tex("(1) Unsorted", color=BLACK)
                        .scale(0.5)
                        .next_to(unsorted_img, DOWN)
                    )
                    sorted_img_caption = (
                        Tex("(2) Sorted", color=BLACK)
                        .scale(0.5)
                        .next_to(sorted_img, DOWN)
                    )
                    images_caption = (
                        Tex(
